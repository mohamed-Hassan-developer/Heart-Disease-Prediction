...
if page == 'Home':
    ...
    Gender_per = col1.radio('Gender', df.Gender.unique(),horizontal=True)

    Smoker_pre = col2.radio('Smoker',options=['No','Yes'],horizontal=True)

    Age_pre = col3.slider('Age',step=1,value=40)

    Fasting_Blood_Sugar_pre = col4.number_input('Fasting Blood Sugar',min_value=40,value=80)
    ...
    Blood_Pressure_pre = col7.number_input('Blood Pressure',min_value=40,value=80 )

    Sleep_Hours_pre = col8.number_input('Sleep Hours',min_value=1.000,max_value=23.000,value=5.000)
    ...
    Triglyceride_Level_pre = col11.number_input('Triglyceride Level',min_value=120.000,max_value=500.000)

    BMI_pre = col12.number_input('BMI',min_value=15.000,max_value=35.000,value=20.000)
    ...
    Family_Heart_Disease_pre =col18.radio('Family Heart History',options=['No','Yes'],horizontal=True)

    CRP_Level_pre = col19.number_input('CRP Level',min_value=0.1,value=5.0)

    Homocysteine_Level_pre = col20.number_input('Homocysteine Level',min_value=5.000,value=8.000)
    ...
    if button == True:
        ...
        if result == 1:
            colP1.error('HD Probabily Positive',width=500,icon="🚨")
            # colP2.write(model.predict_proba(predicted_data).round(3)[0][1] *100)
            colP2.markdown(f"<h1 style='font-size:45px;'>{round(model.predict_proba(predicted_data)[0][1]*100, 2)}%</h1>",unsafe_allow_html=True)


        else:
            colP1.success('HD Nigative',width=500,icon="👍")
            # colP2.write(model.predict_proba(predicted_data).round(3)[0][1] *100)
            colP2.markdown(f"<h1 style='font-size:45px;'>{round(model.predict_proba(predicted_data)[0][1]*100, 2)}%</h1>",unsafe_allow_html=True)
